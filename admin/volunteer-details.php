...
if($_REQUEST['remove']) {
        delete_volunteer($_REQUEST['volunteer_id']);
        Header("Location: manage-vols.php");
}
...
$search['company_id'] = $_REQUEST['volunteer_id'];

$vol = get_volunteer_info($_GET['volunteer_id']);
                ...
                function volEditMode() {
                   ...
                   $("a.save-org").click(function() {
                      ...
                      xhr.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
                       // ROB LOOK HERE; Vol id?
                      xhr.send('volunteer_id=<?php print($vol['volunteer_id']); ?>&vol_name_first=' +           document.getElementById('new_vol_name_first').value + '&vol_name_last=' +
                        document.getElementById('new_vol_name_last').value + '&vol_age=' +
                        document.getElementById('new_vol_age').value + '&vol_email=' + document.getElementById('new_vol_email').value + '&vol_phone=' + document.getElementById('new_vol_phone').value);
                   });
...
<div>
     <form method="POST" id="delete_volunteer" style="margin-top:25px;">
            <input type="hidden" name="remove" value="1" />
            <input type="hidden" name="volunteer_id" value="<?php print($_REQUEST['volunteer_id']);?>" />
            <button type="submit" class="right btn-alert"><span class="fa fa-trash-o" aria-hidden="true"></span> Delete Volunteer</button>
    </form>
